...
const ShowProduct = () => {
  const { filteredData, addCart, token } = useContext(AppContext);
  const navigate = useNavigate();

  const handleAddToCart = async (id, title, price, qty, imgSrc) => {
    if (token) {
      await addCart(id, title, price, qty, imgSrc);
    } else {
  ...
  const handlePrice = async (id, title, price, qty, imgSrc) => {
    if (token) {
      await addCart(id, title, price, qty, imgSrc);
      navigate('/cart');
    }
  ...
  return (
    <div className="product-container">
      ...
      <style jsx="true">{`
        ...
        .price-btn {
          background: #4dabf7; /* Blue for price */
        }
